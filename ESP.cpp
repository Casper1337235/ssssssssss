...
void CEsp::Draw()
{
	...
	for (int i = 0; i < interfaces::ent_list->GetHighestEntityIndex(); i++)
	{
		...
		player_info_t pinfo;
		if (pEntity &&  pEntity != pLocal && !pEntity->dormant())
		{
...
void CEsp::hp_battery(IClientEntity* pEntity, Box size)
{
	...
	Render::DrawRect(flX + 1, flY + size.h + (flMultiplier - rectHeight) + 1, 2, rectHeight, pEntity->dormant() ? Color(0, 0, 0, 140) : Color(0, 250, 90, 255));
...
void CEsp::DrawSkeleton(IClientEntity* pEntity)
{
	...
	for (int j = 0; j < pStudioHdr->numbones; j++)
	{
		...
		if (pBone && (pBone->flags & BONE_USED_BY_HITBOX) && (pBone->parent != -1))
		{
			...
			{
				bool is_ct = pEntity->team() == TEAM_CS_CT;
				Color terrorist = (options::menu.ColorsTab.skeleton_terrorist.GetValue()[0], options::menu.ColorsTab.skeleton_terrorist.GetValue()[1], options::menu.ColorsTab.skeleton_terrorist.GetValue()[2], options::menu.ColorsTab.skeleton_terrorist.GetValue()[3]);
				Color ct = (options::menu.ColorsTab.skeleton_counterterrorist.GetValue()[0], options::menu.ColorsTab.skeleton_counterterrorist.GetValue()[1], options::menu.ColorsTab.skeleton_counterterrorist.GetValue()[2], options::menu.ColorsTab.skeleton_counterterrorist.GetValue()[3]);
				Render::Line(sParent[0], sParent[1], sChild[0], sChild[1], 
