...
void interfaces::Initialise()
{
	...
	c_gamemove = reinterpret_cast<CGameMovement*>(Utilities::Memory::capture_interface("client_panorama.dll", "GameMovement001"));
	c_prediction = reinterpret_cast<IPrediction*>(Utilities::Memory::capture_interface("client_panorama.dll", "VClientPrediction001"));
	...
	movehelper = **(IMoveHelper***)(game_utils::pattern_scan(GetModuleHandleA("client_panorama.dll"), "8B 0D ? ? ? ? 8B 46 08 68") + 2); //test
